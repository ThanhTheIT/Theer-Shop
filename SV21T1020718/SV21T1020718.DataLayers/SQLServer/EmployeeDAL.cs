    ...
    public class EmployeeDAL : BaseDAL, ICommonDAL<Employee>
    {
        ...
        public List<Employee> List(int page = 1, int pageSize = 0, string searchValue = "")
        {
            ...
            using (var connection = OpenConnection())
            {
                ...
                data = connection.Query<Employee>(sql: sql, parameters, commandType: System.Data.CommandType.Text).ToList();
                connection.Close();
            }
            return data;
        }
        ...
        public bool Update(Employee data)
        {
            ...
            using (var connection = OpenConnection())
            {
                var sql = @"if not exists(select * from Employees where EmployeeID <> @EmployeeID and Email = @Email)
                            begin
                                update Employees 
                                set FullName = @FullName, BirthDate = @BirthDate, Address = @Address, Phone = @Phone,
                                    Email = @Email, Password = @Password, Photo = @Photo, IsWorking = @IsWorking
        ...
        public int AddResgister(Employee data)
        {
            throw new NotImplementedException();
        }
    }
