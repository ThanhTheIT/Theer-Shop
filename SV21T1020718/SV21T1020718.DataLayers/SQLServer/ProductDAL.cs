...
b                           else
		...
		public List<Product> ListPB()
		{
			List<Product> data = new List<Product>();
			using (var connection = OpenConnection())
			{
				var sql = @"SELECT TOP (4) [ProductID]
                              ,[ProductName]
                              ,[ProductDescription]
                              ,[SupplierID]
                              ,[CategoryID]
        ...
        public bool UpdateAttribute(ProductAttribute data)
        {
            ...
            {
                var sql = @"
                            if not exists (select * from ProductAttributes where AttributeID <> @AttributeID)
                            begin
        ...
        public bool UpdatePhoto(ProductPhoto data)
        {
            ...
            {
                var sql = @"
                            if not exists (select * from ProductPhotos where PhotoID <> @PhotoID)
                                begin
